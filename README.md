...
- 확장성을 위한 구조적 고찰 (트레이드오프):
  * 하지만 *"인스펙터나 버튼 OnClick 에디터에서 값을 조작하는 것은 번거롭다"*는 생각에 갇히면, 나중에 씬이 늘어났을 때 코드가 반복되는 문제가 생깁니다.
...
- 1. 현재 단계에 맞춘 실용적 구현 (MainMenuController.cs)
...
- 2. 향후 확장성을 고려한 리팩토링 설계 (매개변수 방식 고찰)
