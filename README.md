...
- **메인 화면 씬 추가 및 확장성 고려한 씬 전환 로직**: 메인 화면 씬을 추가하고 버튼 기능 구현 시 함수 내부에 씬 이름을 직접 고정하는 하드코딩 방식과 매개변수를 활용한 동적 씬 전환(재사용성/확장성),
    구조를 비교·검토하여 현재 프로젝트 규모에 최적화된 구조 적용
...
- **UI 및 연출 고도화**:
  * 몬스터 처치 시 머리 위로 획득 골드(+10G) 플로팅 텍스트 연출 추가
